...
def signup(request):
    ...
    try:
        new_user = User.objects.create_user(username = set_email, email=set_email, password=set_password, first_name = set_first_name, last_name = set_last_name,address= set_address,zip_code = set_zip, is_provider = set_is_provider, is_user = set_is_user)
...
def check_user(request):
    if request.user.is_authenticated:
        data = serializers.serialize("json", [request.user], fields=['id','email', 'first_name','last_name','is_provider','pets','address','bases','bio','rate','zip_code'])
...
def add_pet(request):
    ...
    pet = Pet(name = pet_name, species = pet_species, weight = pet_weight, age = pet_age, owner = pet_owner)
    try:
        pet.full_clean()
        pet.save()
    except Exception as e:
...
def connect_pal(request):
    if request.user.is_authenticated:
        ...
        bases = list(request.user.bases.all())
        for base in bases:
            pals.append(list(User.objects.filter(is_provider = True).filter(bases__id = base.id)))
        ...
        for pal in pals:
            ...
            for value in pal:
                ...
                pal_email = {'key': value.pk, 'email' :value.email, 'first_name':value.first_name, 'last_name':value.last_name,'bio':value.bio,'rate':value.rate, 'bases':list(map(lambda base : base.name,value_bases))}
...
def user_detail(request, user_id):
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            data = serializers.serialize("json", [target_user], fields=['email', 'first_name','last_name','is_provider','pets','address','bases','rate','bio','zip_code'],use_natural_foreign_keys=True)
            return HttpResponse(data)
...
def connect_provider(request):
    if request.user.is_authenticated:
        ...
        connect_walker = Walker.objects.all().get(user__email=body["provider"]["email"])
...
def get_provider(request):
    if request.user.is_authenticated:
        ...
        return JsonResponse(model_to_dict(check_walker,fields=['id','email','first_name','last_name','bio','rate']))
...
def schedule_walk(request):
    if request.user.is_authenticated:
        ...
        try:
            walk = Walk(walker = connect_walker, date = date, walk_time = time, owner = request.user)
            walk.full_clean()
            walk.save()
...
def get_pet_walks(request):
    if request.user.is_authenticated:    
        return_list = []
        walks = list(Walk.objects.all().filter(owner = request.user).filter(complete = False))
        for walk in walks:
            walk_pets = list(walk.pets.all())
            return_list.append({'id': walk.id, 'pets':list(map(lambda pet : pet.name, walk_pets)), "date":walk.date, "time":walk.walk_time})
...
def check_connections(request):
    if request.user.is_authenticated:
        return_list = []
        app_users = list(AppUser.objects.all().filter(walker__user = request.user))
        for app_user in app_users:
            return_list.append({'id':app_user.user.id, 'first_name':app_user.user.first_name, 'last_name': app_user.user.last_name, 'email':app_user.user.email, 'address':app_user.user.address})
        return JsonResponse(return_list,safe=False)
...
def get_provider_walks(request):
    if request.user.is_authenticated:
        ...
        walks = list(Walk.objects.all().filter(walker = check_walker).filter(complete = False))
        for walk in walks:
            walk_pets = list(walk.pets.all())
            return_list.append({'id': walk.id, 'pets':list(map(lambda pet : pet.name, walk_pets)), "date":walk.date, "time":walk.walk_time})
        return JsonResponse(return_list, safe = False)
...
def get_walk(request, walk_id):
    if request.user.is_authenticated:
        ...
        return JsonResponse({'id': walk.id, 'pets':list(map(lambda pet : pet.name, walk_pets)), "date":walk.date, "time":walk.walk_time, "owner": name, "notes":walk.notes, "walk_length": walk.walk_length, "walker":walker_name}, safe=False)
...
def get_completed_walks(request):
    if request.user.is_authenticated:
        return_list = []
        walks = list(Walk.objects.all().filter(owner = request.user).filter(complete = True))
        for walk in walks:
            walk_pets = list(walk.pets.all())
            return_list.append({'id': walk.id, 'pets':list(map(lambda pet : pet.name, walk_pets)), "date":walk.date, "time":walk.walk_time})
        return JsonResponse(return_list, safe = False)
...
def pet_detail(request, pet_id):
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            return JsonResponse(model_to_dict(target_pet, fields=['id','name','species','weight','age']))
